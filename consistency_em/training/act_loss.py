...
class _DecoderLayerActivations:
    """Context manager that captures each decoder layer's raw output via forward hooks.

    Captures the raw pre-final-norm output of every decoder layer —
    including the last — rather than the post-norm final hidden state
    that ``output_hidden_states`` substitutes for it, and excludes the
    embedding output. Hooks are registered on enter and removed on exit.
    """
    ...
    def __enter__(self) -> _DecoderLayerActivations:
        base = self._model
...
class ActLoss:
    ...
    def compute(
        ...
        with _DecoderLayerActivations(model) as activations:
            clean_pass(model, clean_inputs)
            clean_activations = activations.snapshot(detach=True)
            activations.clear()
            wrapped_pass(model, wrapped_inputs)
            wrapped_activations = activations.snapshot(detach=False)
