...
class TestRunPhase3Consistency:
    @pytest.fixture
    def fake_consistency_stack(self, monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
        ...
        def fake_peft_load(model: Any, model_id: str, **kwargs: Any) -> str:
            peft_calls.append({"model": model, "model_id": model_id, **kwargs})
            return "<peft-model>"
    ...
    @pytest.fixture
    def make_paired(self) -> Callable[..., Dataset]:
        """Build an act_bct paired dataset with the given number of rows."""

        def _make(rows: int = 2) -> Dataset:
    ...
    def test_disables_remove_unused_columns(
        self,
        fake_consistency_stack: dict[str, Any],
        make_paired: Callable[..., Dataset],
        organism: LoRAAdapter,
